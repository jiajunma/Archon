...
**Prefer existing Mathlib lemmas/definitions.** Do not reintroduce concepts already in Mathlib. Before defining a new structure, typeclass, or definition, search Mathlib thoroughly — it likely already exists under a different name or in a more general form.

**Lean on Mathlib definitions.** If the informal proof's notion matches something in Mathlib, use the Mathlib definition and prove equivalence or provide instances as needed, rather than creating a parallel definition.

**Use mathematically meaningful names.** Avoid problem-specific or ad-hoc names (e.g., `helper1`, `my_lemma`, `foo_aux`). Names should reflect the mathematical content (e.g., `continuous_of_compact`, `integral_add_adjacent_intervals`). This makes lemmas discoverable and reusable.
