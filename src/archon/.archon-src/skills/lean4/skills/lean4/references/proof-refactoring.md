...
## File-Level Modularization

The patterns above cover extracting helpers within a single file. When a **file** grows too large, split it across multiple files.

**Trigger:**
- File exceeds **300–500 lines**, or
- A single lemma's proof exceeds **100 lines** and has extractable prerequisites

**Action:**
1. Create a new file in the same directory (e.g., `MyTheorem_aux.lean`)
2. Move all **fully proven** (no `sorry`, no `axiom`, no errors) lemmas and auxiliary definitions into the new file
3. `import` the new file from the original:
   ```lean
   import MyProject.MyTheorem_aux
   ```

**Rules:**
- Only move code that compiles cleanly — never move code with `sorry` or errors
- Only modularize code **you wrote in the current session**. Do not restructure or move existing, already-compiling code.
- Keep the main theorem in the original file; move prerequisites and helpers to `*_aux.lean`
